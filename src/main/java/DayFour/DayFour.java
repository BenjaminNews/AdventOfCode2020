...
public class DayFour {
    ...
    public int partOne(String input) {
        ...
        for(String passportData : input.split("\n\n")) {
            if(passportData.contains("byr:") && passportData.contains("iyr:") && passportData.contains("eyr:")
                    && passportData.contains("hgt:") && passportData.contains("hcl:") && passportData.contains("ecl:")
    ...
    private boolean isValid(Map<String, String> data) {
        ...
        if(!validateBYR(data.get("byr"))) return false;
        if(!validateIYR(data.get("iyr"))) return false;
        if(!validateEYR(data.get("eyr"))) return false;
        if(!validateHGT(data.get("hgt"))) return false;
        if(!validateHCL(data.get("hcl"))) return false;
        if(!validateECL(data.get("ecl"))) return false;
        if(!validatePID(data.get("pid"))) return false;
        return true;
    }
    ...
    public boolean validateHGT(String s) {
        if(s.contains("in")) {
            int measurement = Integer.parseInt(s.replace("in", ""));
            return measurement >= 59 && measurement <= 76;
        } else if (s.contains("cm")) {
            int measurement = Integer.parseInt(s.replace("cm", ""));
            return measurement >= 150 && measurement <= 193;
        }
        return false;
    }

    public boolean validateHCL(String hairColourValue) {
        return hairColourValue.matches("^(#[0-9a-f]{6})$");
    }

    public boolean validateECL(String eyecolour) {
    ...
    public boolean validatePID(String pidValue) {
