...
public class DaySix {
    ...
    public int partOne(String input) {
        ...
        for(String line : input.split("\n\n")) {
            ...
            line.replaceAll("\n", "").chars().mapToObj(i -> (char) i).forEach(c -> map.put(c, 0));
            result += map.size();
        }
        return result;
    }
    ...
    public int partTwo(String input) {
        int result = 0;
        for(String line : input.split("\n\n")) {
            int personCount = line.split("\n").length;
            Map<Character, Integer> map = new TreeMap<>();
            line.replaceAll("\n", "").chars().mapToObj(i -> (char) i).forEach(c -> map.put(c, 0));
            line.chars().mapToObj(i -> (char) i).forEach(c -> map.computeIfPresent(c, (k, v) -> v + 1));
            result += map.keySet().stream().filter(c -> map.get(c) == personCount).count();
        }
        return result;
    }
